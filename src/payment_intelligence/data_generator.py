...
class PaymentDataGenerator:
    ...
    def generate_all_data(self, output_dir: str = './data') -> Dict[str, pd.DataFrame]:
        ...
        print(f"   ✓ Anonymous users: {users_df['is_anonymous'].sum():,} ({users_df['is_anonymous'].mean()*100:.1f}%)")
        ...
        if len(de_apple_txs) > 0:
            ...
            print(f"   ✓ Pattern 1 (DE + Apple Pay friction):")
        ...
        print(f"   ✓ Pattern 2 (Bitcoin privacy):")
        print(f"     - Bitcoin transactions: {len(btc_txs):,}")
        print(f"     - NULL countries: {btc_txs['country'].isna().sum():,} ({btc_txs['country'].isna().mean()*100:.1f}%)")
        ...
        print(f"   ✓ Pattern 3 (Black Friday spike):")
        print(f"     - November signups: {len(nov_signups):,} ({len(nov_signups)/total_signups*100:.1f}%)")
        print(f"     - Expected baseline: ~8.3% (1/12 months)")
