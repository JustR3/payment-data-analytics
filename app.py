...
def render_executive_overview(analytics):
    ...
    if not acceptance_df.empty:
        ...
        display_df = acceptance_df[['gateway', 'country', 'total_attempts', 
                                    'acceptance_rate_pct', 'soft_decline_rate_pct', 
                                    'hard_decline_rate_pct']].head(10)
        
        st.dataframe(
            display_df.style.map(
                color_acceptance_rate, 
                subset=['acceptance_rate_pct']
            ).format({
...
def render_friction_monitor(analytics):
    ...
    if not friction_df.empty:
        ...
        if not problematic.empty:
            ...
            display_cols = ['gateway', 'country', 'attempts', 'acceptance_rate_pct', 
                          'baseline_rate_pct', 'variance_from_baseline', 
                          'common_errors', 'friction_flag']
...
def render_unit_economics(analytics):
    ...
    if not recon_df.empty:
        # Show table
        display_df = recon_df[['month', 'cash_collected', 'booked_revenue', 
                               'variance', 'variance_pct', 'successful_payments']].head(12)
...
def main():
    ...
    # Sidebar
    with st.sidebar:
        st.image("https://via.placeholder.com/200x60/6d4aff/ffffff?text=Payment+Intel", 
                width='stretch')
